  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.98 }}
        onClick={onSelect}
        className={`relative flex items-center gap-2 px-3 py-2 cursor-pointer rounded-xl transition-all duration-150 ${isActive
  ...
  return (
    <Link href={href}>
      <motion.div
        ...
        whileTap={{ scale: 0.96 }}
        className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs transition-all cursor-pointer ${active
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 min-h-0">
        {chats.length === 0 && !collapsed && (
          ...
          >
            ...
            <p className="text-xs font-semibold text-white/60 mb-1">Mulai percakapan pertamamu</p>
            <motion.button
              whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              onClick={() => { createChat(); onMobileClose?.(); }}
              className="mt-4 px-4 py-2 bg-brand-blue/20 text-brand-blue rounded-xl text-xs font-bold border border-brand-blue/30 shadow-[0_0_15px_rgba(10,132,255,0.2)]"
            >
              Chat Baru
            </motion.button>
          </motion.div>
      ...
      <div className="px-2 pb-2 pt-2 border-t border-white/[0.06] flex-shrink-0">
        <motion.button
          ...
          className={`w-full flex items-center gap-2 py-2 px-3 rounded-xl transition-all border ${hasKey()
    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          dragElastic={0.1}
          onDragEnd={(e, info) => {
            if (info.offset.x < -30) setCollapsed(true);
            if (info.offset.x > 30) setCollapsed(false);
          }}
          className="absolute top-0 right-0 bottom-0 w-2 cursor-col-resize hover:bg-white/10 transition-colors z-20"
        />
      </motion.div>
