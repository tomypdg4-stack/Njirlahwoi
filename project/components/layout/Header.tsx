...
function EditableTitle({ chatId, title }: { chatId: string; title: string }) {
  ...
  return (
    <motion.button
      ...
      className="group flex items-center gap-2 text-white/90 hover:text-white transition-colors max-w-[240px] md:max-w-[400px]"
    >
      <span className="truncate font-semibold text-[15px]">{title === 'New Chat' ? 'Chat Baru' : title}</span>
      <Pencil size={11} className="text-white/20 group-hover:text-brand-blue transition-colors flex-shrink-0 opacity-0 group-hover:opacity-100" />
    </motion.button>
  );
}
  ...
  return (
    ...
    >
      <motion.span
        ...
        animate={clicks >= 3 ? { rotate: [0, 20, -20, 20, -20, 0], scale: [1, 1.4, 1.4, 1.4, 1.4, 1] } : { rotate: 0, scale: 1 }}
    ...
    <div className="flex items-center justify-between px-3 py-2 border-b border-white/[0.06] bg-[#0D0D10]/90 backdrop-blur-xl flex-shrink-0 relative z-10 gap-2">
      ...
      <div className="flex items-center gap-2 min-w-0 flex-1">
        ...
        {activeChat && activeChatId ? (
          ...
          <div className="flex items-center gap-2 min-w-0 ml-2">
            <span className="font-black text-sm font-heading gradient-text whitespace-nowrap">NJIRLAH AI</span>
          </div>
        )}
      </div>
